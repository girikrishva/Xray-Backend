jQuery ->
      ...
      $.ajax url,
        success: (data, status, xhr) ->
          ...
          while i < result.length
            option_line = ('<option value="' + result[i].id + '">' + 'Id: [' + result[i].id + '], Narrative: [' + result[i].narrative + '], Amount: [' + result[i].line_amount + '], Unpaid: [' + result[i].unpaid_amount + ']</option>')
  ...
  $('#payment_line_invoice_line_input').change ->
    ...
    url = '/admin/api/invoice_line_narrative?invoice_line_id=' + escaped_invoice_line_id
    $.ajax url,
      success: (data, status, xhr) ->
